...
class GraphAlgos():
    ...
    def generate_template_path(self, path_rels, source_vertices: list = None):
        path = None
        i = 0
        while path is None and i < 500:
            if source_vertices is None:
                ...
                start = random.choice(source_vertices)
            possible_paths = self.get_queries_for_relpath(path_rels, start)
    ...
    def generate_templte_query(self, path_rels, source_vertices=None):
        path = self.generate_template_path(path_rels, source_vertices)
        if path is None:
            return None
        start = path[0]
        end = path[-1]
        ...
        return self.generate_query_for_path(path), start, correct_answers, path
