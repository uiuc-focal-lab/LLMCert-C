...
BATCH_NUM = 1
GPU_MAP = {0: "0GiB", 1: "0GiB", 2: "20GiB", 3: "20GiB", "cpu":"0GiB"}
INPUT_DEVICE = 'cuda:2'
CONTINUOUS_SAFE = 0
NUM_GEN = 0
...
def get_args():
    ...
    parser.add_argument('--qa_llm', type=str, default='meta-llama/Llama-3.2-11B-Vision-Instruct')
...
def load_model(model_name="meta-llama/Llama-3.2-11B-Vision-Instruct", only_tokenizer=False, gpu_map={0: "26GiB", 1: "0GiB", 2: "0GiB", 3: "0GiB", "cpu":"120GiB"}, quant_type=None):
    tokenizer = AutoTokenizer.from_pretrained(model_name)
    if not only_tokenizer:
        if quant_type is not None:
            if quant_type == '8_bit':
                print("loading 8 bit model")
                model = MllamaForConditionalGeneration.from_pretrained(model_name, device_map='auto', max_memory=gpu_map, torch_dtype=torch.float16, load_in_8bit=True)
            elif quant_type == '4_bit':
                print("loading 4 bit model")
                model = MllamaForConditionalGeneration.from_pretrained(model_name, device_map='auto', max_memory=gpu_map, bnb_4bit_quant_type="nf4", load_in_4bit=True,  bnb_4bit_compute_dtype=torch.float16)
        else:
            print('no quantization, loading in fp16')
            model = MllamaForConditionalGeneration.from_pretrained(model_name, device_map='auto', max_memory=gpu_map, torch_dtype=torch.float16, attn_implementation='eager')
...
def query_model(prompts, model, tokenizer, do_sample=True, top_k=10, 
    ...
    chats = []
    if len(prompts) > 1:
        raise ValueError("Only one prompt is allowed for this model")
    else:
        chats = [{"role": "system", "content": LLAMA3_SYS_PROMPT}, {"role":"user", "content":[{"type":"text", "text":f"{prompts[0]}"}]}]
        
    input_text = tokenizer.apply_chat_template(chats,  add_generation_prompt=True, tokenize=False)
    input_ids = tokenizer(input_text, return_tensors='pt').input_ids.to(INPUT_DEVICE)
